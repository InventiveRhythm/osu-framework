...
{
    internal static class DrawTypePerformanceMonitor
    {
        ...
        private const string group_name = "Draw (Top consumers)";
        ...
        public static void NewFrame()
        {
            ...
            if (framesSinceLastReport > 0 && clock.CurrentTime - lastReport > 1000)
            {
                ...
                GlobalStatistics.Get<string>(group_name, $"0. Total").Value =
                    $"{collected_times.Sum(x => x.Value.TotalMilliseconds) / framesSinceLastReport:N1}ms";

                int i = 0;

                foreach (var t in collected_times.OrderByDescending(t => t.Value.TotalMilliseconds).Take(9))
                {
